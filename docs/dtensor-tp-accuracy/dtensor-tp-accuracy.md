...
<img src=".\Fig\token_mult_prob_error_qwen3_4B.png" alt="The token_mult_pob_error for Qwen3-4B" style="zoom:33%;" />

<center style="color:#C0C0C0;">  Fig 1:The token_mult_pob_error of Qwen3-4B </center>
...
$$
ratio & = & exp(\text{current\_logprobs} - \text{prev\_logprobs}) \\
& = & exp(log(\frac{\text{current\_probs}}{\text{prev\_probs}})) \\
...
2. Column-wise parallel (TP = 2)
   ...
   In **column-wise** parallelism, we split W*W* by columns (output dimension) into two blocks:
   ...
   d_{\text{in}}^{(1)} + d_{\text{out}}^{(2)} = d_{\text{out}}.
   $$
...
Below is an example of how the default plan can be adjusted into a more numerically stable configuration. More deails can refer to [NeMo-RL PR! 1235](https://github.com/NVIDIA-NeMo/RL/pull/1235)
...
- <!--**Batch‑invariant and TP‑invariant kernels** that enforce consistent reduction trees across GPUs to achieve bit‑wise identical results under different TP sizes (e.g., [Tree‑Based Invariant Kernels, TBIK](https://arxiv.org/abs/2511.17826)).-->
- <!--**[Deterministic kernel and backend configurations](https://www.ingonyama.com/post/solving-reproducibility-challenges-in-deep-learning-and-llms-our-journey)**, where attention and matmul kernels are selected to be shape‑fixed and deterministic across runs and hardware variants, improving reproducibility at the cost of some performance.-->
